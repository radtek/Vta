...
ImageCropper::ImageCropper(const QString& name ,QWidget* parent) :
	QWidget(parent),
...
void ImageCropper::keyPressEvent(QKeyEvent *event)
{
    switch (event->key())
    {
         case Qt::Key_Return:
         {
        QPixmap p =   cropImage();
         p.save(qApp->applicationDirPath()+"/VtaImage/"+Name1);
          this->hide();
              break;
             }
         default:
